...
export class QQBotChannel implements Channel {
  ...
  // Track last message ID per chat for passive replies
  ...
  private async reconnect(): Promise<void> {
    try {
      ...
      const url: string = gwRes.data.url;
    ...
    switch (op) {
      ...
      case OP_DISPATCH:
        if (t === 'READY') {
          ...
          console.log('\n  QQ Bot connected');
          console.log('  Send messages to the bot to get chat JIDs\n');
        } else if (t === 'RESUMED') {
          this.connected = true;
  ...
  private sendIdentify(): void {
    this.send({
      ...
      d: {
        ...
        intents: INTENTS,
        shard: [0, 1],
        properties: { $os: 'linux', $browser: 'medclaw', $device: 'medclaw' },
      },
    });
  }
  ...
  async sendMessage(jid: string, text: string): Promise<void> {
    try {
      ...
      // Include passive reply IDs to avoid active message limits
...
registerChannel('qq', (opts: ChannelOpts) => {
  ...
  if (!appId || !appSecret) {
    logger.warn('QQ Bot: QQ_BOT_APP_ID or QQ_BOT_APP_SECRET not set');
    return null;
  }
